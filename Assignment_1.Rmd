...
dflong <- data.frame(lapply(dflong,function(x) {x <- gsub(".","999",x,fixed=TRUE)})) 

myvars <- c("vote") 

newdf<-dflong[which( dflong$vote != 999),]#delete all with 999 in vote

aggregate(dflong[myvars], by=list(Party=dflong$Party, PA=dflong$Pro_Ant, Year=dflong$year), mean)
